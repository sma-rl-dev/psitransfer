...
const storeErrors = require('./tusboy/store/errors');
...
class Store {
  ...
  async append(fid, readStream, offset) {
    ...
    // NOTE: Node >= 22 rejects createWriteStream with {flags: 'a', start}
    // (ENOTSUP: operation not supported on socket, write). The tus client
    // always resumes at Upload-Offset == current size, so plain append is
    // positionally correct; reject anything else with OffsetMismatch (409).
    const currentSize = await fsp.stat(this.getFilename(fid))
      .then(stat => stat.size, e => {
        if (e.code === 'ENOENT') return 0;
        throw e;
      });
    if (currentSize !== offset) throw new storeErrors.OffsetMismatch();
    const ws = fs.createWriteStream(this.getFilename(fid), {flags: 'a'});
